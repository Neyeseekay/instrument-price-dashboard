...
- **Shareable, refresh-safe ticker selection.** The selected tickers live in the URL (`?tickers=AAPL,MSFT`), kept in sync with the app state in both directions — pasting the link reproduces the same comparison view for someone else, a refresh doesn't lose the selection, and browser back/forward work as expected. A stale or hand-edited link silently drops any ticker the backend doesn't recognize (`frontend/src/app/useUrlTickerSync.ts`).
